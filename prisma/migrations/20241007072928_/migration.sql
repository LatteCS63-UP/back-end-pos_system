/*
  ...
  - Made the column `ProvinceID` on table `Shop` required. This step will fail if there are existing NULL values in that column.
  - Made the column `DistrictID` on table `Shop` required. This step will fail if there are existing NULL values in that column.
  - Made the column `SubDistrictID` on table `Shop` required. This step will fail if there are existing NULL values in that column.

*/
-- DropIndex
DROP INDEX `Shop_DistrictID_fkey` ON `Shop`;

-- DropIndex
DROP INDEX `Shop_ProvinceID_fkey` ON `Shop`;

-- DropIndex
DROP INDEX `Shop_SubDistrictID_fkey` ON `Shop`;
...
-- AlterTable
ALTER TABLE `MasJobTitle` MODIFY `CreateDate` DATETIME NOT NULL,
    MODIFY `LastDate` DATETIME NULL;
...
-- AlterTable
ALTER TABLE `Shop` MODIFY `ProvinceID` INTEGER NOT NULL,
